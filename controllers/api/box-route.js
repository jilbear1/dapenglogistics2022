...
router.get("/dq_count", withAuth, async (req, res) => {
  try {
    ...
    const xc_containers = containerData.map((container) =>
...
// this api is for admin force status change from received to shipped due to the reason of discarding the boxes, therefore
//no shipping charge shall be included in the upcoming bill, only the sotraging fee
router.put("/toStatusShippedById", withAuth, (req, res) => {
  Box.update(
    {
      status: 3,
      shipped_date: req.body.shipped_date,
      bill_shipped: new Date(req.body.shipped_date).getTime() + 86400000,
