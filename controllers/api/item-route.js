  ...
  try {
    const itemData = await Item.findOne({
      ...
      include: [
        ...
        {
          model: Container,
          attributes: ["container_number", "id"],
        },
        {
          model: User,
          attributes: ["name", "id"],
...
router.get(
  "/allItemPerNumberAdmin/:itemArry&:client_id",
  withAuth,
  async (req, res) => {
    try {
      var itemArray = req.params.itemArry;
...
router.put(
  "/updateQty_ExistedItem/:container_id&:item_number",
  withAuth,
  (req, res) => {
    Item.update(
      {
...
router.put(
  "/updateQty_ExistedItemId/:container_id&:item_id",
  withAuth,
  (req, res) => {
    Item.update(
      {
...
router.get(
  "/itemValidation/:item_number&:container_id",
  withAuth,
  async (req, res) => {
    try {
      const itemData = await Item.findOne({
        ...
        attributes: ["container_id", "item_number", "id", "qty_per_sku"],
      });
      var data;
...
router.put(
  `/rewireClientRequest/:item_id&:container_id&:parentAccountId`,
  withAuth,
  (req, res) => {
    Item.update(
      {
...
router.get("/modification/:sku", withAuth, async (req, res) => {
  try {
    const skuData = await Item.findAll({
      where: {
        item_number: req.params.sku,
      },
      attributes: ["item_number", "qty_per_sku", "id"],
      include: [
        {
          model: Container,
          where: {
    ...
    if (skuData) {
      const skus = skuData.map((sku) => sku.get({ plain: true }));
      res.json(skus);
    } else {
      res.status(404);
    }
