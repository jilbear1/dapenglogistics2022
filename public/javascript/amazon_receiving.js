...
function containerChecker(cNumber) {
  fetch(`/api/container/amazon_container/${cNumber}`, {
    ...
    .then(function (data) {
      if (
        data.id &&
        client_list.value == data.user_id &&
        accountSelect.value == data.account_id
      ) {
        alert(
          "This box is already assocaited with this account! Please use refill mode to add more items"
        );
        let code = prompt(
          "Please enter the passcode to override the restriction"
        );
        if (code == "0523") {
          checker = true;
        ...
        alert(
          `This box is already associated with the account: ${data.account.name}! Please use a new amazon box #`
        );
        let code = prompt(
          "Please enter the passcode to override the restriction"
        );
        ...
        alert(
          `This box is already associated with a client ${data.user.name}! Please use a new amazon box #`
        );
        container_number.value = null;
        disableButton();
      }
...
function amazonCreate() {
  if (
    itemCount < 1 &&
    !confirm("No item was scanned! Continue to create an empty amazon box?")
  ) {
    return;
  }
...
function itemCreate() {
  ...
  promises.push(
    record_container(amazon_box, itemCollection, itemCollectionCount)
  );
  Promise.all(promises)
    .then(() => {
      loader.style.display = "none";
      alert(
        `1 container(#${amazon_box.container_number}) with ${itemCount} items is inserted to client_id: ${amazon_box.user_id}!`
      );
      resetBoxSku();
    })
    .catch((e) => {
...
function masterCheck() {
  ...
  if (
    ...
    width_d &&
    height_d &&
    container_d.substring(0, 2).toUpperCase() == "AM" &&
    container_d.length == 8 &&
    client_list_d &&
    account_d &&
    validation(client_list_d, account_d)
  ) {
...
function itemInput() {
  ...
  if (
    (skuValue.substring(0, 1) == "X" && skuValue.length == 10) ||
    (skuValue.length > 2 && skuValue.substring(0, 1) != "-")
  ) {
...
async function duplicatationValidator(obj) {
  const promises = [];
  await fetch(
    `/api/item/itemValidation/${obj.item_number}&${obj.container_id}`,
    {
      method: "GET",
    }
  )
...
async function updateExistedItem(obj, newSkuQ) {
  const load = await fetch(
    `/api/item/updateQty_ExistedItem/${obj.container_id}&${obj.item_number}`,
    {
      method: "PUT",
      body: JSON.stringify({
        qty_per_sku: newSkuQ,
    ...
    .then(function (data) {
      ...
      fetch(`/api/container/allContainerAdmin`, {
        ...
        .then(function (data) {
          ...
          console.log(
            `other empty boxes ${testOtherArr.length}: ` + testOtherArr
          );
          if (!emptyArr.length) {
