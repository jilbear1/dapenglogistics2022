console.log(location.href, 'rmaster_req_amazon js');
  ...
  if (skuColContentArr.length < 1) {
    ...
    skuColContentArr.forEach(element => {
      if(element.length>1) {
        ...
        item.innerHTML = `<div class="col-4">${element.trim()}</div> <div class="col-4"><input type="number" value=${element.trim().split("(#")[1].split(")")[0]} class="uk-input uk-form-width-small uk-form-small text-danger" onkeyup="checkSKU(event, ${element.trim().split("(#")[1].split(")")[0]})"></div>`;
        skuCol.prepend(item)
      }
    });
  }
...
const checkSKU = (event, number) => {
  if (parseInt(event.target.value)<=parseInt(number) && parseInt(event.target.value)>0) {
    event.target.className = "uk-input uk-form-width-small uk-form-small text-danger";
  } else {
