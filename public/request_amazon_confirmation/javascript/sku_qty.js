    ...
    .then(function (data) {
      ...
      for (let i = 1; i < tr.length; i++) {
        ...
        if (containerMap.get(its_container_id)) {
          tr[i].getElementsByTagName("td")[6].innerHTML = Math.ceil(
            parseFloat(tr[i].getElementsByTagName("td")[6].innerText) * 2.2
          );
          var sizeData = tr[i]
          ...
          tr[i].getElementsByTagName(
            "td"
          )[5].innerHTML = `${sizeData[0].toFixed(1)} x ${sizeData[1].toFixed(
            1
          )} x ${sizeData[2].toFixed(1)}`;
...
const palletSwitch = (ev) => {
  ...
  if (ev.target.id == "ind") {
    ...
    ev.target.id = "req";
    ev.target.className =
      "btn btn-white border-danger text-danger btn-sm shadow-sm";
    ...
    ev.target.className =
      "btn btn-white border-success text-success btn-sm shadow-sm";
  } else {
    ...
    ev.target.id = "ind";
    ev.target.className =
      "btn btn-white border-primary text-primary btn-sm shadow-sm";
  }
};
...
function selectBatch(tracking, ev) {
  accountName =
    ev.target.parentNode.parentElement.querySelectorAll("td")[1].innerText;
  if (!selectIndividual) {
    var allSiblingBoxes;
    selectPerPallet
      ? (allSiblingBoxes = document.getElementsByClassName(tracking))
      : (allSiblingBoxes = siblingTracker(tracking));
    for (let i = 0; i < allSiblingBoxes.length; i++) {
      const eachCheckBox = allSiblingBoxes[i].getElementsByTagName("input")[0];
      const eachContainerId = parseInt(
        eachCheckBox.parentElement.parentElement
          .getAttribute("id")
          .split("_")[1]
      );
      !selectBoxId.includes(eachContainerId)
        ? selectBoxId.push(eachContainerId)
        : console.log(eachContainerId + " already in the array");
      eachCheckBox.parentElement.setAttribute(
        "class",
    ...
    if (tracking.includes("_")) {
      if (!ev.target.checked) {
        ...
        for (let i = 0; i < allSiblingBoxes.length; i++) {
          const eachCheckBox =
            allSiblingBoxes[i].getElementsByTagName("input")[0];
          const eachContainerId = parseInt(
            eachCheckBox.parentElement.parentElement
              .getAttribute("id")
              .split("_")[1]
      ...
      if (ev.target.checked) {
        const individualId = parseInt(
          ev.target.parentElement.parentElement.getAttribute("id").split("_")[1]
        );
...
function GetSelected(numberOfFile) {
  if (selectBoxId.length) {
    document.getElementById("js-modal-confirm").innerHTML =
    ...
    for (let i = 0; i < selectBoxId.length; i++) {
      var itemCollection = "Colletion: ";
      var total_skus = 0;
      const sku_collection = containerMap
        .get(selectBoxId[i])
        .forEach((item) => {
      ...
      promises.push(
        record_container(
          total_skus,
          itemCollection,
          containerRef.get(selectBoxId[i]),
          s3,
          numberOfFile
        )
      );
...
function validation_request() {
  ...
  if (!file && !file_2 && !check_label.checked) {
    alert(
      "The shipping label is missing! Please attach a pdf file and try again! 无夹带档案！请夹带档案或者勾选无夹带档案栏，然后再试一遍。"
    );
  } else {
...
const preCheckPage = (file, file_2, numberOfFile) => {
  ...
  for (var i = 0; i < allInputs.length; i++) {
    if (allInputs[i].checked) {
      ...
      var qty_per_sku = "<ul>";
      const spBoxNumber =
        firstTd.nextElementSibling.nextElementSibling.innerText;
      const spSKUArr =
        firstTd.nextElementSibling.nextElementSibling.nextElementSibling.innerText.split(
          "\n"
        );
      spSKUArr.forEach((i) => (spSKU += `<li>${i}</li>`));
      const qty_per_skuArr =
        firstTd.nextElementSibling.nextElementSibling.nextElementSibling.nextElementSibling.innerText.split(
          "\n"
        );
  ...
  for (let i = 1; i < row.length; i++) {
    ...
    if (element) {
      ...
      if (!non_repeatArr.includes(uuid)) {
        row[
          i
        ].cells[8].innerHTML = `托盘号<b>${new_index}</b>-${pallet_id}: ${pallet_length} x ${pallet_width} x ${pallet_height}, ${pallet_weight}`;
